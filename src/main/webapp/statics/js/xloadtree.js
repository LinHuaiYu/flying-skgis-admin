...
function WebFXLoadTree(sText, sXmlSrc, sAction, sBehavior, sIcon, sOpenIcon, sWid) {
	// call super
	this.WebFXTree = WebFXTree;
	this.WebFXTree(sText, sAction, sBehavior, sIcon, sOpenIcon, sWid);
...
function WebFXLoadTreeItem(sText, sXmlSrc, sAction, eParent, sIcon, sOpenIcon,sWid) {
	// call super

	this.WebFXTreeItem = WebFXTreeItem;
	this.WebFXTreeItem(sText, sAction, eParent, sIcon, sOpenIcon, sWid);
...
function _1111startLoadXmlTree(sSrc, jsNode) {
	if (jsNode.loading || jsNode.loaded)
		return;
		jsNode.loading = true;
		    ...
		    xmlHttp.send();
		    _xmlFileLoaded(xmlHttp.responseXML, jsNode);
		   // xml = unescape(xmlHttp.responseText);
		   // alert(xml);
		    //Doc.loadXML(xml);
		}	
...
function _xmlTreeToJsTree(oNode) {
	...
	var wid = oNode.getAttribute("wid");
	//alert(wid);
	// create jsNode
	var jsNode;
	if (src != null && src != "")
		jsNode = new WebFXLoadTreeItem(text, src, action, parent, icon, openIcon,wid);
	else
	    jsNode = new WebFXTreeItem(text, action, parent, icon, openIcon, wid);
